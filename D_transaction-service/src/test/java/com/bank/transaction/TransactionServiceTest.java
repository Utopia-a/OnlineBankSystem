...
class TransactionServiceTest {
    ...
    void deposit_success() {
        Account account = buildAccount(1001L, new BigDecimal("5000.00"), "ACTIVE");
    ...
    void deposit_frozenAccount_throws() {
        Account account = buildAccount(1001L, new BigDecimal("5000.00"), "FROZEN");
        when(accountRepository.findByIdWithLock(1001L)).thenReturn(Optional.of(account));
    ...
    void withdraw_insufficientBalance_throws() {
        Account account = buildAccount(1001L, new BigDecimal("100.00"), "ACTIVE");
        when(accountRepository.findByIdWithLock(1001L)).thenReturn(Optional.of(account));
        when(transactionRepository.sumAmountByAccountAndTypeAndStatusBetween(
                anyLong(), anyList(), any(), any(), any()))
                .thenReturn(BigDecimal.ZERO);
    ...
    void withdraw_exceedDailyLimit_throws() {
        Account account = buildAccount(1001L, new BigDecimal("100000.00"), "ACTIVE");
        when(accountRepository.findByIdWithLock(1001L)).thenReturn(Optional.of(account));
    ...
    void transfer_success() {
        Account fromAccount = buildAccount(1001L, new BigDecimal("5000.00"), "ACTIVE");
        Account toAccount   = buildAccount(1002L, new BigDecimal("1000.00"), "ACTIVE");
    ...
    private Account buildAccount(Long id, BigDecimal balance, String status) {
        Account a = new Account();
        a.setId(id);
        a.setAccountNo("ACC" + id);
